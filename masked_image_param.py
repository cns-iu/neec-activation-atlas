...
class MaskedImageParam(ImageParam):
    def __init__(self, mask_tensor, image=None, device="cuda"):
        ...
        if image is None:
            ...
            if self.width % 2 == 1:
                self.param = init_image_param(
                    height=self.height, width=self.width + 1, sd=0.01, device=device
                )
            else:
                self.param = init_image_param(
                    height=self.height, width=self.width, sd=0.01, device=device
                )
            img = fft_to_rgb(
                height=self.height,
                width=self.width,
        ...
        assert (
            self.mask.shape[-2:] == self.to_nchw_tensor(device=self.device).shape[-2:]
        ), (
            "The height and width of the input image"
            + str(self.to_nchw_tensor(device=self.device).shape[-2:])
            + "and the mask"
            + str(self.mask.shape[-2:])
            + "do not match."
        )

    def to_chw_tensor(self, device="cpu"):
        t = self.forward(device=device).squeeze(0).clamp(
            0, 1
        ).detach() + self.original_nchw_image_tensor.to(device) * (
            1 - self.mask.to(device)
        )
        return t.squeeze(0)

    def forward(self, device):
        return self.normalize(self.postprocess(device=device), device=device).clamp(
            0, 1
        ) * self.mask.to(device)
    ...
    def update_mask(self, mask):
        ...
        self.original_nchw_image_tensor = self.to_chw_tensor(
            device=self.device
        ).unsqueeze(0)
