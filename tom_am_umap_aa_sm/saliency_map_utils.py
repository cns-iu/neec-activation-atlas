...
from hook_utils import Hook, default_func_mean
...
class Dreamer:
    ...
    def render_saliency(
        ...
        """
        fe = FeatureExtractor(self.model, layers)
        ...
        mask = self.model(img)
        layer_outputs = fe(img)[layers[0]]
        res = (mask*layer_outputs).sum()
        res.backward()
    ...
    def get_snapshot(self, layers, input_tensor):
        ...
        with torch.no_grad():
            ...
            layer_outputs = fe(input_tensor)[layers[0]]
